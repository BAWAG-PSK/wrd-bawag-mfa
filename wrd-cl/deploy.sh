...
deploy () {
  ...
  az storage blob delete-batch -s "\$web" --pattern wrd-cl --connection-string ${AZ_STORAGE_DEV}
  az storage blob upload-batch -d "\$web/wrd-cl" -s dist --connection-string ${AZ_STORAGE_DEV}
}

npm run build:prod
# npm run build:prod:mock # build the mocked production version (ext.-build)
deploy
