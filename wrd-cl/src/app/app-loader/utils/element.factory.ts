...
export class ElementFactory {
  ...
  public static createScriptElement(script: Script, resolve, reject): HTMLScriptElement {
    ...
    scriptElement.async = script.async === true; // false = does not pause HTML parsing after script has been downloaded
    scriptElement.defer = script.defer !== false; // true = script will not run until after the page has loaded
    if (script.type) {
  ...
  public static createLinkElement(style: Style, resolve, reject): HTMLLinkElement {
    ...
    linkElement.onload = () => resolve(style);
