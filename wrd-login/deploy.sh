...
deploy () {
  ...
  az storage blob delete-batch -s "\$web" --pattern wrd-login --connection-string ${AZ_STORAGE_DEV}
  az storage blob upload-batch -d "\$web/wrd-login" -s dist --connection-string ${AZ_STORAGE_DEV}
}
