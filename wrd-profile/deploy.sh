...
deploy () {
  ...
  az storage blob delete-batch -s "\$web" --pattern wrd-profile --connection-string ${AZ_STORAGE_DEV}
  az storage blob upload-batch -d "\$web/wrd-profile" -s dist --connection-string ${AZ_STORAGE_DEV}
}
