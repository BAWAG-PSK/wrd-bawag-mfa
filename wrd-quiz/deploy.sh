...
deploy () {
  ...
  az storage blob delete-batch -s "\$web" --pattern wrd-quiz --connection-string ${AZ_STORAGE_DEV}
  az storage blob upload-batch -d "\$web/wrd-quiz" -s dist --connection-string ${AZ_STORAGE_DEV}
}
