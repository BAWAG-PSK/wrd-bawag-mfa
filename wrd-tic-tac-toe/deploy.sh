...
deploy () {
  ...
  az storage blob delete-batch -s "\$web" --pattern wrd-tic-tac-toe --connection-string ${AZ_STORAGE_DEV}
  az storage blob upload-batch -d "\$web/wrd-tic-tac-toe" -s dist --connection-string ${AZ_STORAGE_DEV}
}
